...
# TestMediaStorage = get_storage_class(settings.DEFAULT_FILE_STORAGE)
...
class CURRENCY:
    ...
    @classmethod
    def to_list(cls):
        return [CURRENCY.INR, CURRENCY.EUR, CURRENCY.USD]
...
class NGOReportItem(TimeStampedModel):
    ...
    amount = models.FloatField(_("Amount spent (INR)"))
...
class NGOHelper(TimeStampedModel):
    ...
    resource_image = models.ImageField(_("Resource Images"), max_length=300, blank=True,help_text=_("Images should be  more than 500x500px and should be of good quality"))
    # resource_image= models.TextField(_("Resource Images"))
...
class RegisterNGORequest(TimeStampedModel):
    ...
    description = models.TextField(
        _("Description"), max_length=500,blank=True, help_text=_("Organization's short description - max 500 chars.")
    )
    ...
    resource_types = models.TextField(
        _("Resource tags"), max_length=500,blank=True, help_text=_("The types of resources you anticipate you will need.")
    )

    contact_name = models.CharField(_("Contact person's name"), blank=True,max_length=254)
    email = models.EmailField(_("Email"), blank=True, default="")
    contact_phone = models.CharField(_("Contact person's phone"), blank=True,max_length=15)
    has_netopia_contract = models.BooleanField(_("Has contract with Netopia/PayTM"), blank=True, default=False)
    address = models.CharField(_("Address"), blank=True,max_length=400)
    city = models.CharField(_("City"),blank=True, max_length=100)
    county = models.CharField(_("State"), blank=True, choices=COUNTY.to_choices(), max_length=50)
    ...
    last_balance_sheet = models.FileField(
        _("First page of last balance sheet"),blank=True, max_length=300, storage=PrivateMediaStorageClass()
    )
    statute = models.FileField(_("NGO Statute"), max_length=300, blank=True, storage=PrivateMediaStorageClass())

    registered_on = models.DateTimeField(_("Registered on"), blank=True, auto_now_add=True)
