...
class PaymentOrder(TimeStampedModel):
    ...
    first_name = models.CharField(_("First name"), blank=True,max_length=254)
    last_name = models.CharField(_("Last name"),blank=True, max_length=254)
    show_name = models.BooleanField(
        default=False, verbose_name="I agree that my name will appear on the DoNation platform "
    )
    phone = models.CharField(_("Phone"), max_length=30,blank=True)
    email = models.EmailField(_("Email"),blank=True)
    address = models.CharField(_("Address"), max_length=254,blank=True)
    details = models.TextField(_("Details"),blank=True)
    amount = models.FloatField(_("Amount"),blank=True)
    date = models.DateTimeField(_("Registered on"), blank=True,auto_now_add=True)
